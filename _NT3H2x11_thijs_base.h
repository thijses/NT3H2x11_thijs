...
#ifndef _NT3H2x11_thijs_base_h
#define _NT3H2x11_thijs_base_h

#include "Arduino.h" // always import Arduino.h

#include "NT3H2x11_thijs.h" // (i feel like this constitutes a cicular dependency, but the compiler doesn't seem to mind)


#ifndef NT3H2x11_useWireLib // (note: ifNdef!) if this has been defined by the user, then don't do all this manual stuff
    ...
    #warning("using Wire library for NT3H2x11 (no platform optimized code available)")
    #define NT3H2x11_useWireLib  // use Wire as a backup
  #endif
#endif

#ifdef NT3H2x11_useWireLib // note: this ifdef check is done after the ifndef, so the compiler gets a chance to define it anyway
  #include <Wire.h>
...
otherwise, just a boolean, indicating general faillure should be enough, especially if you use NT3H2x11debugPrint() effectively
...
*/
#ifndef NT3H2x11_ERR_RETURN_TYPE  // unless the user already defined it manually
  #define NT3H2x11_ERR_RETURN_TYPE_default  bool
  #define NT3H2x11_ERR_RETURN_TYPE_default_OK  true
  #define NT3H2x11_ERR_RETURN_TYPE_default_FAIL false
  #ifdef NT3H2x11_useWireLib
    #define NT3H2x11_ERR_RETURN_TYPE  NT3H2x11_ERR_RETURN_TYPE_default
    #define NT3H2x11_ERR_RETURN_TYPE_OK  NT3H2x11_ERR_RETURN_TYPE_default_OK
    #define NT3H2x11_ERR_RETURN_TYPE_FAIL  NT3H2x11_ERR_RETURN_TYPE_default_FAIL
  #elif defined(ARDUINO_ARCH_ESP32) // the ESP32 likes to spit out esp_err_t for most things
    #define NT3H2x11_ERR_RETURN_TYPE  esp_err_t
    #define NT3H2x11_ERR_RETURN_TYPE_OK  ESP_OK
    #define NT3H2x11_ERR_RETURN_TYPE_FAIL  ESP_FAIL
    #define NT3H2x11_return_esp_err_t   // to let the code below know that the return type is an esp_err_t
  #elif defined(ARDUINO_ARCH_STM32)
    #define NT3H2x11_ERR_RETURN_TYPE  i2c_status_e
    #define NT3H2x11_ERR_RETURN_TYPE_OK  I2C_OK
    #define NT3H2x11_ERR_RETURN_TYPE_FAIL  I2C_ERROR // NOTE: this may cause some debugging confusion (but then again, STM32 I2C is already hard to debug. Just try using WireLib)
    #define NT3H2x11_return_i2c_status_e // to let the code below know that the return type is an esp_err_t
  #else
    #error("failed to define ERR_RETURN_TYPE for some reason")
  #endif
#endif
#ifndef NT3H2x11_ERR_RETURN_TYPE_OK
  #warning("NT3H2x11_ERR_RETURN_TYPE was manually defined, but NT3H2x11_ERR_RETURN_TYPE_OK was not!, attempting to infer automatically...")
  #ifdef NT3H2x11_return_esp_err_t
    #define NT3H2x11_ERR_RETURN_TYPE_OK  ESP_OK
  #elif defined(NT3H2x11_return_i2c_status_e)
    #define NT3H2x11_ERR_RETURN_TYPE_OK  I2C_OK
  #else
    #error("when manually defining the NT3H2x11_ERR_RETURN_TYPE, please also define the NT3H2x11_ERR_RETURN_TYPE_OK. See _NT3H2x11_thijs_base.h")
  #endif
#endif
#ifndef NT3H2x11_ERR_RETURN_TYPE_FAIL
  #warning("NT3H2x11_ERR_RETURN_TYPE was manually defined, but NT3H2x11_ERR_RETURN_TYPE_FAIL was not!, attempting to infer automatically...")
  #ifdef NT3H2x11_return_esp_err_t
    #define NT3H2x11_ERR_RETURN_TYPE_FAIL  ESP_FAIL
  #elif defined(NT3H2x11_return_i2c_status_e)
    #define NT3H2x11_ERR_RETURN_TYPE_FAIL  I2C_ERROR // NOTE: this may cause some debugging confusion (but then again, STM32 I2C is already hard to debug. Just try using WireLib)
  #else
    #error("when manually defining the NT3H2x11_ERR_RETURN_TYPE, please also define the NT3H2x11_ERR_RETURN_TYPE_FAIL. See _NT3H2x11_thijs_base.h")
  #endif
#endif
...
/**
 * (this is only the base class, users should use NT3H2x11_thijs)
 * 
 */
class _NT3H2x11_thijs_base
{
  public:
  //// I2C constants:
  ...
  _NT3H2x11_thijs_base(bool is2kVariant, uint8_t address=NT3H2x11_DEFAULT_I2C_ADDRESS) : is2kVariant(is2kVariant), slaveAddress(address) {}
  
  #ifdef NT3H2x11_useWireLib // higher level generalized (arduino wire library):
     ...
     * @param readBuff a NT3H2x11_BLOCK_SIZE buffer to store the read values in
    ...
    bool requestMemBlock(uint8_t blockAddress, uint8_t readBuff[]) {
      ...
      return(_onlyReadBytes(readBuff, NT3H2x11_BLOCK_SIZE));
    }
     ...
     * @return whether it wrote/read successfully
     */
    bool requestSessRegByte(NT3H2x11_CONF_SESS_REGS_ENUM registerIndex, uint8_t& readBuff) {
      // ideally, i'd use the Wire function: requestFrom(address, quantity, iaddress, isize, sendStop), which lets you send the register through iaddress
      // HOWEVER, this function is not implemented on all platforms (looking at you, MSP430!), and it's not that hard to do manually anyway, so:
      Wire.beginTransmission(slaveAddress);
      Wire.write(NT3H2x11_SESS_REGS_MEMA);
    ...
    bool _onlyReadBytes(uint8_t readBuff[], uint8_t bytesToRead) {
      ...
      if(Wire.available() != bytesToRead) { NT3H2x11debugPrint("onlyReadBytes() received insufficient data"); return(false); }
     ...
     * @param bytesToWrite how many bytes of actual data to write, the remainder (to complete the NT3H2x11_BLOCK_SIZE block) will be 0's
     * @return whether it wrote successfully
     */
    bool writeMemBlock(uint8_t blockAddress, uint8_t writeBuff[], uint8_t bytesToWrite=NT3H2x11_BLOCK_SIZE) {
      if(bytesToWrite > NT3H2x11_BLOCK_SIZE) {/* PANIC */  NT3H2x11debugPrint("writeMemBlock() can only write in blocks of 16 bytes, not more!"); return(false); }
      Wire.beginTransmission(slaveAddress);
      Wire.write(blockAddress);
      Wire.write(writeBuff, bytesToWrite); // (usually) just calls a forloop that calls .write(byte) for every byte.
      for(uint8_t i=0; i<(NT3H2x11_BLOCK_SIZE-bytesToWrite); i++) { Wire.write(0); } // pad 0's to make the block complete
     ...
     * @return whether it wrote successfully
     */
    bool writeSessRegByte(NT3H2x11_CONF_SESS_REGS_ENUM registerIndex, uint8_t regDat, uint8_t mask=0xFF) {
      Wire.beginTransmission(slaveAddress);
      Wire.write(NT3H2x11_SESS_REGS_MEMA);
      Wire.write(registerIndex);
      Wire.write(mask);
    ...
    //   NT3H2x11debugPrint("softReset() doesn't work with WireLib, as it is not meant for sending repeated starts.");
    // }
    ...
    */
    ...
    inline bool startWrite() {
      ...
      if(twoWireStatusReg != twi_SR_M_SLA_W_ACK) { NT3H2x11debugPrint("SLA_W ack error"); TWCR = twi_STOP; return(false); }
      return(true);
    }
    ...
    inline bool startRead() {
      ...
      if(twoWireStatusReg != twi_SR_M_SLA_R_ACK) { NT3H2x11debugPrint("SLA_R ack error"); TWCR = twi_STOP; return(false); }
      return(true);
    }
    ...
    bool requestSessRegByte(NT3H2x11_CONF_SESS_REGS_ENUM registerIndex, uint8_t& readBuff) {
      if(!startWrite()) { return(false); }
      twiWrite(NT3H2x11_SESS_REGS_MEMA);  //if(twoWireStatusReg != twi_SR_M_DAT_T_ACK) { return(false); } //should be ACK(?)
    ...
    bool _onlyReadBytes(uint8_t readBuff[], uint8_t bytesToRead) {
      ...
      for(uint8_t i=0; i<(bytesToRead-1); i++) {
        ...
        twoWireTransferWait();
        //if(twoWireStatusReg != twi_SR_M_DAT_R_ACK) { NT3H2x11debugPrint("DAT_R Ack error"); return(false); }
        readBuff[i] = TWDR;
      }
      TWCR = twi_basic; //request 1 more byte
      twoWireTransferWait();
      //if(twoWireStatusReg != twi_SR_M_DAT_R_NACK) { NT3H2x11debugPrint("DAT_R Nack error"); return(false); }
    ...
    bool writeMemBlock(uint8_t blockAddress, uint8_t writeBuff[], uint8_t bytesToWrite=NT3H2x11_BLOCK_SIZE) {
      ...
      for(uint8_t i=0; i<NT3H2x11_BLOCK_SIZE; i++) {
  ...
  #elif defined(ARDUINO_ARCH_ESP32)
    // see my NT3H2x11 library for notes on the ESP32's mediocre I2C peripheral
    ...
    esp_err_t init(uint32_t frequency, int SDApin=21, int SCLpin=22, i2c_port_t I2CportToUse = 0) {
      if(I2CportToUse < I2C_NUM_MAX) { I2Cport = I2CportToUse; } else { NT3H2x11debugPrint("can't init(), invalid I2Cport!"); return(ESP_ERR_INVALID_ARG); }
      i2c_config_t conf;
      ...
      if (err != ESP_OK) { NT3H2x11debugPrint("can't init(), i2c_param_config error!"); NT3H2x11debugPrint(esp_err_to_name(err)); return(err); }
     ...
     */
    NT3H2x11_ERR_RETURN_TYPE requestMemBlock(uint8_t blockAddress, uint8_t readBuff[]) {
      // for alternate commands (manually selected I2C operations), please refer to AS5600_thijs or TMP112_thijs
      esp_err_t err = i2c_master_write_read_device(I2Cport, slaveAddress, &blockAddress, 1, readBuff, NT3H2x11_BLOCK_SIZE, I2Ctimeout / portTICK_RATE_MS); //faster (seems to work fine)
      if(err != ESP_OK) { NT3H2x11debugPrint(esp_err_to_name(err)); }
      #ifdef NT3H2x11_return_esp_err_t
        return(err);
      #else
        return(err == ESP_OK);
     ...
     * @return (esp_err_t or bool) whether it wrote/read successfully
     */
    NT3H2x11_ERR_RETURN_TYPE requestSessRegByte(NT3H2x11_CONF_SESS_REGS_ENUM registerIndex, uint8_t& readBuff) {
      // for alternate commands (manually selected I2C operations), please refer to AS5600_thijs or TMP112_thijs
      uint8_t requestArr[2] = {NT3H2x11_SESS_REGS_MEMA, registerIndex};
     ...
     */
    NT3H2x11_ERR_RETURN_TYPE _onlyReadBytes(uint8_t readBuff[], uint8_t bytesToRead) {
      // for alternate commands (manually selected I2C operations), please refer to AS5600_thijs or TMP112_thijs
     ...
     */
    NT3H2x11_ERR_RETURN_TYPE writeMemBlock(uint8_t blockAddress, uint8_t writeBuff[], uint8_t bytesToWrite=NT3H2x11_BLOCK_SIZE) {
      if(bytesToWrite > NT3H2x11_BLOCK_SIZE) {/* PANIC */  NT3H2x11debugPrint("writeMemBlock() can only write in blocks of 16 bytes, not more!"); return(NT3H2x11_ERR_RETURN_TYPE_FAIL); }
      esp_err_t err;
      // if(bytesToWrite == NT3H2x11_BLOCK_SIZE) {
      ...
      // } else { // probably slightly slower:
        uint8_t copiedArray[NT3H2x11_BLOCK_SIZE+1]; copiedArray[0]=blockAddress; for(uint8_t i=0;i<NT3H2x11_BLOCK_SIZE;i++) { copiedArray[i+1]=(i<bytesToWrite) ? writeBuff[i] : 0; }
        err = i2c_master_write_to_device(I2Cport, slaveAddress, copiedArray, NT3H2x11_BLOCK_SIZE+1, I2Ctimeout / portTICK_RATE_MS);
      // }
      if(err != ESP_OK) { NT3H2x11debugPrint(esp_err_to_name(err)); }
      #ifdef NT3H2x11_return_esp_err_t
        return(err);
     ...
     * @return (esp_err_t or bool) whether it wrote successfully
     */
    NT3H2x11_ERR_RETURN_TYPE writeSessRegByte(NT3H2x11_CONF_SESS_REGS_ENUM registerIndex, uint8_t regDat, uint8_t mask=0xFF) {
      // const uint8_t numberOfCommands = 7; //start, write, write, write, write, write, stop
      // uint8_t CMDbuffer[SIZEOF_I2C_CMD_DESC_T + SIZEOF_I2C_CMD_LINK_T * numberOfCommands] = { 0 };
      // i2c_cmd_handle_t cmd = i2c_cmd_link_create_static(CMDbuffer, sizeof(CMDbuffer)); //create a CMD sequence
      // i2c_master_start(cmd);
      // i2c_master_write_byte(cmd, (slaveAddress<<1) | TW_WRITE, ACK_CHECK_EN);
      // i2c_master_write_byte(cmd, NT3H2x11_SESS_REGS_MEMA, ACK_CHECK_DIS); // TODO: ACK checks...
      ...
      // i2c_cmd_link_delete_static(cmd);
      //// hopefully faster:
      uint8_t regWriteArr[4] = {NT3H2x11_SESS_REGS_MEMA, registerIndex, mask, regDat};
    ...
    void init(uint32_t frequency) {
      ...
      // the default module is all i'm going to need for my uses, but if you wanted to use multiple I2C peripherals, please uncomment all the twi_setModule() things and add a module constant to each NT3H2x11_thijs obj
      twi_init();
    ...
    bool requestMemBlock(uint8_t blockAddress, uint8_t readBuff[]) {
      ...
      if(ret != 0) { NT3H2x11debugPrint("requestSessRegByte() twi_writeTo error!"); return(false); }
      return(_onlyReadBytes(readBuff, NT3H2x11_BLOCK_SIZE));
    }
    ...
    bool _onlyReadBytes(uint8_t readBuff[], uint8_t bytesToRead) {
      ...
      if(readQuantity != bytesToRead) { NT3H2x11debugPrint("_onlyReadBytes() received insufficient data"); return(false); }
      return(true);
    }
    ...
    bool writeMemBlock(uint8_t blockAddress, uint8_t writeBuff[], uint8_t bytesToWrite=NT3H2x11_BLOCK_SIZE) {
      ...
      if(bytesToWrite > NT3H2x11_BLOCK_SIZE) {/* PANIC */  NT3H2x11debugPrint("writeMemBlock() can only write in blocks of 16 bytes, not more!"); return(false); }
      uint8_t copiedArray[NT3H2x11_BLOCK_SIZE+1]; copiedArray[0]=blockAddress; for(uint8_t i=0;i<NT3H2x11_BLOCK_SIZE;i++) { copiedArray[i+1]=(i<bytesToWrite) ? writeBuff[i] : 0; }
      int8_t ret = twi_writeTo(slaveAddress, copiedArray, NT3H2x11_BLOCK_SIZE+1, 1, true); // transmit some bytes, wait for the transmission to complete and send a STOP command
      if(ret != 0) { NT3H2x11debugPrint("writeMemBlock() twi_writeTo error!"); return(false); }
      return(true);
    ...
    bool writeSessRegByte(NT3H2x11_CONF_SESS_REGS_ENUM registerIndex, uint8_t regDat, uint8_t mask=0xFF) {
      ...
      if(ret != 0) { NT3H2x11debugPrint("writeSessRegByte() twi_writeTo error!"); return(false); }
      return(true);
    }
    ...
    */
    ...
    NT3H2x11_ERR_RETURN_TYPE requestMemBlock(uint8_t blockAddress, uint8_t readBuff[]) {
      ...
      if(err != I2C_OK) {
        NT3H2x11debugPrint("requestReadBytes() i2c_master_write error!");
        #ifdef NT3H2x11_return_i2c_status_e
          return(err);
        #else
          return(false);
        #endif
    ...
    NT3H2x11_ERR_RETURN_TYPE _onlyReadBytes(uint8_t readBuff[], uint8_t bytesToRead) {
      ...
      if(err != I2C_OK) { NT3H2x11debugPrint("onlyReadBytes() i2c_master_read error!"); }
      #ifdef NT3H2x11_return_i2c_status_e
        return(err);
      #else
        return(err == I2C_OK);
    ...
    NT3H2x11_ERR_RETURN_TYPE writeMemBlock(uint8_t blockAddress, uint8_t writeBuff[], uint8_t bytesToWrite=NT3H2x11_BLOCK_SIZE) {
      ...
      #endif
      uint8_t copiedArray[NT3H2x11_BLOCK_SIZE+1]; copiedArray[0]=blockAddress; for(uint8_t i=0;i<NT3H2x11_BLOCK_SIZE;i++) { copiedArray[i+1]=(i<bytesToWrite) ? writeBuff[i] : 0; }
      i2c_status_e err = i2c_master_write(_i2c, (slaveAddress << 1), copiedArray, NT3H2x11_BLOCK_SIZE+1);
      if(err != I2C_OK) { NT3H2x11debugPrint("writeMemBlock() i2c_master_write error!"); }
      #ifdef NT3H2x11_return_i2c_status_e
        return(err);
      #else
        return(err == I2C_OK);
    ...
    NT3H2x11_ERR_RETURN_TYPE writeSessRegByte(NT3H2x11_CONF_SESS_REGS_ENUM registerIndex, uint8_t regDat, uint8_t mask=0xFF) {
      ...
      if(err != I2C_OK) { NT3H2x11debugPrint("writeSessRegByte() i2c_master_write error!"); }
      #ifdef NT3H2x11_return_i2c_status_e
        return(err);
      #else
        return(err == I2C_OK);
  ...
  // /**
  //  * write a whole block (using a dedicated struct). Just a macro to writeMemBlock(), as &_blockStruct returns a uint8_t&
  //  * @param blockAddress MEMory Address (MEMA) of the block
  ...
  //  */
  // NT3H2x11_ERR_RETURN_TYPE _writeBlockStruct(uint8_t blockAddress, _blockStruct& blockToWrite) { return(writeMemBlock(blockAddress, &blockToWrite)); } // (just a macro)

  /*
  the remainder of the code can be found in the main header file: NT3H2x11_thijs.h
  This is just a parent class, meant to hold all the low-level I2C implementations
  */
};


#endif // _NT3H2x11_thijs_base_h