...
// #define NT3H2x11_useWireLib  // force the use of Wire.h (instead of platform-optimized code (if available))

#define NT3H2x11debugPrint(x)  Serial.println(x)    //you can undefine these printing functions no problem, they are only for printing I2C errors
//#define NT3H2x11debugPrint(x)  log_d(x)  // ESP32 style logging

// #define NT3H2x11_unlock_burning

#include <NT3H2x11_thijs.h>

NT3H2x11_thijs NFCtag(  false  ); // initialize 1k variant


#ifdef ARDUINO_ARCH_ESP32  // on the ESP32, almost any pin can become an I2C pin
  const uint8_t NT3H2x11_SDApin = 26; // 'defualt' is 21 (but this is just some random value Arduino decided.)
  const uint8_t NT3H2x11_SCLpin = 27; // 'defualt' is 22 
#endif
#ifdef ARDUINO_ARCH_STM32   // on the STM32, each I2C peripheral has several pin options
  const uint8_t NT3H2x11_SDApin = SDA; // default pin, on the STM32WB55 (nucleo_wb55rg_p) that's pin PB9
  const uint8_t NT3H2x11_SCLpin = SCL; // default pin, on the STM32WB55 (nucleo_wb55rg_p) that's pin PB8
...
#ifdef NT3H2x11_useWireLib // (currently) only implemented with Wire.h
...
void setup() 
{
  ...
  #ifdef NT3H2x11_useWireLib // the slow (but pretty universally compatible) way
...
//    pinMode(NT3H2x11_SDApin, INPUT_PULLUP); //not needed, as twoWireSetup() does the pullup stuff for you
//    pinMode(NT3H2x11_SCLpin, INPUT_PULLUP);
    esp_err_t initErr = NFCtag.init(100000, NT3H2x11_SDApin, NT3H2x11_SCLpin, 0); //on the ESP32 (almost) any pins can be I2C pins
  ...
  #else
    #error("should never happen, NT3H2x11_useWireLib should have automatically been selected if your platform isn't one of the optimized ones")
  #endif
  ...
  if(!NFCtag.connectionCheck()) { Serial.println("NT3H2x11 connection check failed!");    while(1);    } else { Serial.println("connection good"); }
  ...
  Serial.print("  should be: "); Serial.println(NT3H2x11_CAPA_CONT_DEFAULT_uint32_t[NFCtag.is2kVariant], HEX);
  if(!NFCtag.variantCheck()) { Serial.println("NT3H2x11 variant check failed!");    while(1);     } else  { Serial.println("variant good"); }
  Serial.println(); // seperator

  // //// address change test:
  // #ifdef NT3H2x11_useWireLib
  //   I2CdebugScan();
  //   uint8_t newAddress = NT3H2x11_DEFAULT_I2C_ADDRESS;
  ...
  //   I2CdebugScan();
  //   if(!NFCtag.connectionCheck()) { Serial.println("post-address-change NT3H2x11 connection check failed!"); while(1);} else { Serial.println("post-address-change connection good"); }
  // #endif

  // //// endian-ness test?
  // uint32_t testVal = 0x04030201;
  // uint8_t* bytePtrToNewVal = (uint8_t*) &testVal;
  // Serial.println("endian-ness test:");
  // Serial.print(testVal); Serial.print('\t'); Serial.print(testVal, HEX); Serial.print('\t'); Serial.println(testVal, BIN);
  // for(uint8_t i=0; i<sizeof(testVal); i++) {
  //   Serial.print(bytePtrToNewVal[i], HEX); Serial.print('\t');
  ...
  Serial.print("getSess_NC_REG: 0b"); Serial.print(NFCtag.getSess_NC_REG(), BIN); Serial.print("  default: 0b"); Serial.println(NT3H2x11_SESS_REGS_DEFAULT[0], BIN);
  ...
  Serial.println(); // seperator
  Serial.print("getSess_LAST_NDEF_BLOCK: 0x"); Serial.print(NFCtag.getSess_LAST_NDEF_BLOCK(), HEX); Serial.print("  default: 0x"); Serial.println(NT3H2x11_SESS_REGS_DEFAULT[1], HEX);
  Serial.print("getSess_SRAM_MIRROR_BLOCK: 0x"); Serial.print(NFCtag.getSess_SRAM_MIRROR_BLOCK(), HEX); Serial.print("  default: 0x"); Serial.println(NT3H2x11_SESS_REGS_DEFAULT[2], HEX);
  ...
  Serial.print("getConf_NC_REG: 0b"); Serial.print(NFCtag.getConf_NC_REG(), BIN); Serial.print("  default: 0b"); Serial.println(NT3H2x11_CONF_REGS_DEFAULT[0], BIN);
  Serial.print("getConf_NC_NFCS_I2C_RST: "); Serial.println(NFCtag.getConf_NC_NFCS_I2C_RST());
  Serial.print("getConf_NC_PTHRU: "); Serial.println(NFCtag.getConf_NC_PTHRU()); // (NOTE: 1x01 to 2x11 change)
  ...
  Serial.print("getConf_NC_MIRROR: "); Serial.println(NFCtag.getConf_NC_MIRROR()); // (NOTE: 1x01 to 2x11 change)
  Serial.print("getConf_NC_DIR: "); Serial.println(NFCtag.getConf_NC_DIR());
  Serial.println(); // seperator
  Serial.print("getConf_LAST_NDEF_BLOCK: 0x"); Serial.print(NFCtag.getConf_LAST_NDEF_BLOCK(), HEX); Serial.print("  default: 0x"); Serial.println(NT3H2x11_CONF_REGS_DEFAULT[1], HEX);
  Serial.print("getConf_SRAM_MIRROR_BLOCK: 0x"); Serial.print(NFCtag.getConf_SRAM_MIRROR_BLOCK(), HEX); Serial.print("  default: 0x"); Serial.println(NT3H2x11_CONF_REGS_DEFAULT[2], HEX);
  ...
  Serial.print("getConf_I2C_CLOCK_STR: "); Serial.print(NFCtag.getConf_I2C_CLOCK_STR()); Serial.print("  default: "); Serial.println(NT3H2x11_CONF_REGS_DEFAULT[5]);
  Serial.println(); // seperator
  Serial.print("getREG_LOCK: 0b"); Serial.print(NFCtag.getREG_LOCK(), BIN); Serial.print("  default: 0b"); Serial.println(NT3H2x11_CONF_REGS_DEFAULT[6], BIN);
  Serial.println(); // seperator
...
//   for(uint8_t i=0; i<6; i++) { Serial.print(testOne._data[i+NT3H2x11_PWD_MEMA_BYTES_START], HEX); Serial.print('\t'); } Serial.println();
//   for(uint8_t i=0; i<6; i++) { Serial.print(testTwo._data[i+NT3H2x11_PWD_MEMA_BYTES_START], HEX); Serial.print('\t'); } Serial.println();
//   for(uint8_t i=0; i<6; i++) { Serial.print(testThree._data[i+NT3H2x11_PWD_MEMA_BYTES_START], HEX); Serial.print('\t'); } Serial.println();
//   for(uint8_t i=0; i<6; i++) { Serial.print(testFour._data[i+NT3H2x11_PWD_MEMA_BYTES_START], HEX); Serial.print('\t'); } Serial.println();
//   Serial.println();
  ...
  //requestMemBlock(blockAddress, yourBuffHere);     which is the same as    _getBytesFromBlock(blockAddress, 0, NT3H2x11_BLOCK_SIZE, yourBuffHere);    for reading blocks
  //writeMemBlock(blockAddress, yourBuffHere);       which is the same as      _setBytesInBlock(blockAddress, 0, NT3H2x11_BLOCK_SIZE, yourBuffHere);    for writing blocks
  //// to read/write WHOLE blocks of memory efficiently, just make sure your readBuff/writeBuff is the size of NT3H2x11_BLOCK_SIZE. The _getBytes...() functions will recognize this and skip the _oneBlockBuff
  ...
  //// for obvious reasons, these One-Time-Program functions are not available in this library UNLESS, you #define NT3H2x11_unlock_burning
