...
- I2C_CLOCK_STR: I2C clock stretching enable/disable {NOTE: also contains NEG_AUTH_REACHED, indicating whether AUTHLIM is reached}
...
TODO (general):
- find proper CC bytes for 2k version (indicate size correctly) (datasheet only mentions 0x6D )
- merge NT3H1x01 and this library in a clean and efficient way (not very difficult, just tricky, as NT3H2x11 adds password functions)
...
#ifndef NT3H2x11_thijs_h
#define NT3H2x11_thijs_h

#include "Arduino.h"


//#define NT3H2x11_unlock_burning   // enable the permanent chip-burning features of the NT3H2x11 (e.g. REG_LOCK)

//#define NT3H2x11debugPrint(x)  Serial.println(x)
//#define NT3H2x11debugPrint(x)  log_d(x)   //using the ESP32 debug printing

#ifndef NT3H2x11debugPrint
  #define NT3H2x11debugPrint(x)  ;
#endif


//// NT3H2x11 constants:

#define NT3H2x11_DEFAULT_I2C_ADDRESS 0x55

#define NT3H2x11_BLOCK_SIZE 16 // the I2C interface only deals in blocks of 16 bytes (except for Session registers)

#define NT3H2x11_I2C_ADDR_CHANGE_MEMA 0x00      // I2C address change memory block
#define NT3H2x11_I2C_ADDR_CHANGE_MEMA_BYTE  0   // I2C address change covers byte 0 only (write only)

#define NT3H2x11_SERIAL_NR_MEMA 0x00            // Serial Number memory block
#define NT3H2x11_SERIAL_NR_MEMA_BYTES_START 0   // Serial Number covers bytes 0~6 (read only)
#define NT3H2x11_SERIAL_NR_NXP_MF_ID 0x04       // Serial Number manufacturer number (of NXP). Should be the first byte of UID on all NXP tags

#define NT3H2x11_SAK_MEMA 0x00                  // SAK memory block
#define NT3H2x11_SAK_MEMA_BYTE         7        // SAK covers byte 7 (read only)
#define NT3H2x11_ATQA_MEMA 0x00                 // ATQA memory block
#define NT3H2x11_ATQA_MEMA_BYTES_START 8        // ATQA covers bytes 8~9 (read only)

#define NT3H2x11_CAPA_CONT_MEMA 0x00            // Capability Container memory block
#define NT3H2x11_CAPA_CONT_MEMA_BYTES_START 12  // Capability Container covers bytes 12~15
static const uint32_t NT3H2x11_CAPA_CONT_DEFAULT_uint32_t[2] = {0xE1106D00, 0xE110EA00};  // Capability Container factory default value (as uint32_t) for the {1k, 2k} variant
static const uint8_t  NT3H2x11_CAPA_CONT_DEFAULT[2][4] = {{0xE1,0x10,0x6D,0x00}, {0xE1,0x10,0xEA,0x00}}; // Capability Container factory default value (as individual bytes) for the {1k, 2k} variant

#define NT3H2x11_STAT_LOCK_MEMA 0x00            // Static Locking bytes memory block
#define NT3H2x11_STAT_LOCK_MEMA_BYTES_START 10  // Static Locking covers bytes 10~11

#define NT3H2x11_DYNA_LOCK_MEMA 0x38            // Dynamic Locking bytes memory block (NOTE: 1x01 to 2x11 change)
#define NT3H2111_DYNA_LOCK_RFUI_bits 0xFF0F3F00   // Dynamic Locking RFUI mask for the 1k variant (RFU = Reserved for Future Use)
#define NT3H2211_DYNA_LOCK_RFUI_bits 0xFFFFFF00   // Dynamic Locking RFUI mask for the 2k variant (NOTE: 1x01 to 2x11 change)

//// password stuff: (NOTE: 1x01 to 2x11 change)
...
#define NT3H2x11_CONF_REGS_MEMA 0x3A            // Configuration registers memory block (NOTE: 1x01 to 2x11 change)
#define NT3H2x11_SESS_REGS_MEMA 0xFE            // Session registers memory block (NOTE: can only be accessed using special read/write operation, see pages 37~38 of datasheet)

#define NT3H2x11_INVALID_MEMA   0xFF // ther are several invalid memory block addresses, but this is the most recognisable one
// other invalid addresses include: 0x3B~(0xF7@1k~0x3F@2k), (0x80@2k~0xF7@2k), 0xFC~0xFD

enum NT3H2x11_CONF_SESS_REGS_ENUM : uint8_t { // you could also do this with just defines, but it's slightly fancier this way
//// configuration/session registers common:
  NT3H2x11_COMN_REGS_NC_REG_BYTE = 0,  // NC_REG register location in the conf/sess. registers
  NT3H2x11_COMN_REGS_LAST_NDEF_BLOCK_BYTE = 1,  // LAST_NDEF_BLOCK register location in the conf/sess. registers
...
//// session registers only:
  NT3H2x11_SESS_REGS_NS_REG_BYTE = 6  // NS_REG register location in the conf/sess. registers
};

//// default values of Configuration and Session registers, as defined in the datasheet:
static const uint8_t NT3H2x11_CONF_REGS_DEFAULT[7] = {0x01, 0x00, 0xF8, 0x48,
                                                      0x08, 0x01, 0x00}; // the 8th byte is a fixed 0
static const uint8_t NT3H2x11_SESS_REGS_DEFAULT[7] = {0x01, 0x00, 0xF8, 0x48,
                                                      0x08, 0x01, 0x00};

static const float NT3H2x11_WDT_RAW_TO_MICROSECONDS = 9.43; // multiply the (uint16_t) WDT regsiters value with this to get microseconds

enum NT3H2x11_FD_ON_ENUM : uint8_t { // 2bit value to determine what triggers the FD pin to activate (sink LOW)
  NT3H2x11_FD_ON_FIELD_PRESENCE = 0, // RF field present
  NT3H2x11_FD_ON_START_OF_COMM  = 1, // start of communication (presumably triggers before anticollision (multiple tags)????)
  NT3H2x11_FD_ON_TAG_SELECTED   = 2, // this tag being selected (not sure what the difference between this and communication start are...)
  NT3H2x11_FD_ON_PTHRU_MODE     = 3  // (in Pass-Through mode) data in buffer: ready-to-be-read-by-I2C OR has-been-read-by-RF
};
enum NT3H2x11_FD_OFF_ENUM : uint8_t { // 2bit value to determine what triggers the FD pin to deactivate (pull HIGH)
  NT3H2x11_FD_OFF_FIELD_PRESENCE = 0, // RF field gone
  NT3H2x11_FD_OFF_HALT           = 1, // (RF field gone or) tag is set to HALT state
  NT3H2x11_FD_OFF_LAST_NDEF_READ = 2, // (RF field gone or) last block of data has been read (defined by LAST_NDEF_BLOCK)
  NT3H2x11_FD_OFF_PTHRU_MODE     = 3  // (in Pass-Through mode and FD_ON set to NT3H2x11_FD_ON_PTHRU_MODE) (RF field gone or) data in buffer: has-been-read-by-I2C OR ready-to-be-read-by-RF
};
...
#define NT3H2x11_NC_REG_MIRROR_bits   0b00000010 // SRAM_MIRROR_ON_OFF enables Memory-Mirror mode
#define NT3H2x11_NC_REG_DIR_bits      0b00000001 // TRANSFER_DIR/PTHRU_DIR determines the direction of data in Pass-Through mode, or can disable RF write-access otherwise
//// NOTE: NT3H1x01 had RFUs for NT3H2x11_NC_REG_PTHRU_bits and NT3H2x11_NC_REG_MIRROR_bits in conf registers.
////       Also, those bits now reset to 0 in the session registers under certain circumstances. See page 29 of datasheet
...
#define NT3H2x11_NC_REG_LOCK_I2C_bits 0b00000010 // PTHRU_ON_OFF enables Pass-Through mode
#define NT3H2x11_NC_REG_LOCK_RF_bits  0b00000001 // SRAM_MIRROR_ON_OFF enables Memory-Mirror mode
//// NS_REG (Session only):
#define NT3H2x11_NS_REG_NDEF_READ_bits  0b10000000 // NDEF_DATA_READ flag is 1 once the RF interface has read the data at address LAST_NDEF_BLOCK (if set). Reading clears flag
#define NT3H2x11_NS_REG_I2C_LOCKED_bits 0b01000000 // I2C_LOCKED is 1 if I2C has control of memory (arbitration). Should be cleared once the I2C interaction is completely done, may be cleared by WDT
#define NT3H2x11_NS_REG_RF_LOCKED_bits  0b00100000 // RF_LOCKED is 1 if RF has control of memory (arbitration)
#define NT3H2x11_NS_REG_PTHRU_IN_bits   0b00010000 // SRAM_I2C_READY is 1 if data is ready in SRAM buffer to be READ by I2C (i'm not sure if checking this flag changes it)
#define NT3H2x11_NS_REG_PTHRU_OUT_bits  0b00001000 // SRAM_RF_READY is 1 if data is ready in SRAM buffer to be READ by RF (the I2C should not need to check this flag, and i'm not sure if checking it clears it)
#define NT3H2x11_NS_REG_EPR_WR_ERR_bits 0b00000100 // EEPROM_WR_ERR is 1 if there was a (High Voltage?) error during EEPROM write. Flag needs to be manually cleared
#define NT3H2x11_NS_REG_EPR_WR_BSY_bits 0b00000010 // EEPROM_WR_BUSY is 1 if EEPROM writing is in progress (access is disabled while writing)
#define NT3H2x11_NS_REG_RF_FIELD_bits   0b00000001 // RF_FIELD_PRESENT is 1 if an RF field is detected
...
struct passwordBlock : public _blockStruct { // a struct specifically for the password block, just to make sure it's all written correctly (and you don't lock yourself out)
  ...
  { for(uint8_t i=0;i<NT3H2x11_BLOCK_SIZE;i++){_data[i]=0;}  ACCESS() = ACCESS_in;  PWD_uint32_t() = PWD_in;  PACK_uint16_t() = PACK_in;  PT_I2C() = PI_I2C_in; } // (just a macro)
  passwordBlock(uint8_t ACCESS_in, uint8_t PWD_buff_in[], uint8_t PACK_buff_in[], uint8_t PI_I2C_in) // constructor with byte arrays
  { for(uint8_t i=0;i<NT3H2x11_BLOCK_SIZE;i++){_data[i]=0;}  ACCESS() = ACCESS_in;  for(uint8_t i=0;i<4;i++){PWD(i)=PWD_buff_in[i];}  for(uint8_t i=0;i<2;i++){PACK(i)=PACK_buff_in[i];}  PT_I2C() = PI_I2C_in; }
  //// bit functions:
...
/* other blocks i could make:
- Configuration registers
- MEMA 0x00 (addr, UID, static_lock, CC)
*/


#include "_NT3H2x11_thijs_base.h" // this file holds all the nitty-gritty low-level stuff (I2C implementations (platform optimizations))
/**
 * An I2C interfacing library for the NT3H2x11 NFC IC
 * 
 */
class NT3H2x11_thijs : public _NT3H2x11_thijs_base
{
  public:
  //private:
  uint8_t _oneBlockBuff[NT3H2x11_BLOCK_SIZE]; // used for user-friendly functions. A cache of 1 block of memory, to be used whenever less-than-a-whole-block is to be changed (less memory assignment overhead)
  uint8_t _oneBlockBuffAddress = NT3H2x11_INVALID_MEMA; // indicates the memory address the _oneBlockBuff stores. Use with great caution, and only if speed is an absolute necessity!
  uint8_t _passwordAndPackBuff[6] = {0}; // the password and PACK (password acknowledge) cannot be Read, so overwriting anything in block 0x39 will overwrite the password and PACK
  bool _passwordAndPackStored = false; // useful for generating debug print messages (in case you're overwriting a password unintentionally)
  public:
  using _NT3H2x11_thijs_base::_NT3H2x11_thijs_base;
  /*
  This class only contains the higher level functions.
   for the base functions, please refer to _NT3H2x11_thijs_base.h
  ...
  */
  ...
  /**
   * (just a macro) check whether an NT3H2x11_ERR_RETURN_TYPE (which may be one of several different types) is fine or not 
   * @param err (bool or esp_err_t or i2c_status_e, see on defines at top)
   * @return whether the error is fine
   */
  bool _errGood(NT3H2x11_ERR_RETURN_TYPE err) { return(err == NT3H2x11_ERR_RETURN_TYPE_OK); }
  //   #if defined(NT3H2x11_return_esp_err_t)     // these few commented lines were replaced with the one above, but still serve to show how the error system works:
  //     return(err == ESP_OK);
  //   #elif defined(NT3H2x11_return_i2c_status_e)
  //     return(err == I2C_OK);
  //   #else
  //     return(err);
  //   #endif
   ...
   */
  NT3H2x11_ERR_RETURN_TYPE _getBytesFromBlock(uint8_t blockAddress, uint8_t bytesInBlockStart, uint8_t bytesToRead, uint8_t readBuff[], bool useCache=false) {
    if((bytesInBlockStart+bytesToRead) > NT3H2x11_BLOCK_SIZE) { NT3H2x11debugPrint("_getBytesFromBlock() MISUSE!, you're trying to read bytes outside of the buffer"); return(NT3H2x11_ERR_RETURN_TYPE_FAIL); }
        // bytesInBlockStart=(bytesInBlockStart%NT3H2x11_BLOCK_SIZE); bytesToRead=min((uint8_t)(NT3H2x11_BLOCK_SIZE-bytesInBlockStart), bytesToRead); } // safety measures instead of return()
    uint8_t* whichBuffToUse = _oneBlockBuff; // default to class member block
    if((bytesToRead == NT3H2x11_BLOCK_SIZE) && (bytesInBlockStart == 0)) { whichBuffToUse = readBuff; } // ONLY IF readBuff is actually a full block's worth of data, then you can read directly to it (skip copying)
    ...
    if( ! (useCache && (blockAddress == _oneBlockBuffAddress) && (whichBuffToUse == _oneBlockBuff))) { // normally true
      ...
      if(!_errGood(err)) { NT3H2x11debugPrint("_getBytesFromBlock() read/write error!"); _oneBlockBuffAddress = NT3H2x11_INVALID_MEMA; return(err); }
    }
  ...
  T _getValFromBlock(uint8_t blockAddress, uint8_t bytesInBlockStart, bool readMSBfirst=true, bool useCache=false) {
    T returnVal;
    if((bytesInBlockStart+sizeof(T)) > NT3H2x11_BLOCK_SIZE) { NT3H2x11debugPrint("_getValFromBlock() MISUSE!, you're trying to read bytes outside of the buffer"); return(NT3H2x11_ERR_RETURN_TYPE_FAIL);  }
    NT3H2x11_ERR_RETURN_TYPE err = NT3H2x11_ERR_RETURN_TYPE_OK;
    if( ! (useCache && (blockAddress == _oneBlockBuffAddress))) { // normally true
      ...
      if(!_errGood(err)) { NT3H2x11debugPrint("_getValFromBlock<>() read/write error!"); _oneBlockBuffAddress = NT3H2x11_INVALID_MEMA; return(returnVal); } // note: returns random value, as returnVal was not zero-initialized
    }
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE _setBytesInBlock(uint8_t blockAddress, uint8_t bytesInBlockStart, uint8_t bytesToWrite, uint8_t writeBuff[], bool useCache=false) {
    uint8_t* whichBuffToUse = _oneBlockBuff; // default to class member block
    if((bytesToWrite == NT3H2x11_BLOCK_SIZE) && (bytesInBlockStart == 0)) { whichBuffToUse = writeBuff; } // ONLY IF readBuff is actually a full block's worth of data, then you can read directly to it (skip copying)
    NT3H2x11_ERR_RETURN_TYPE err = NT3H2x11_ERR_RETURN_TYPE_OK;
    if(whichBuffToUse == _oneBlockBuff) { // if the writeBuff is not an entire block worth of data, you must first read the block (to fill in the gaps when you send it back)
      if( ! (useCache && (blockAddress == _oneBlockBuffAddress))) { // normally true
        err = requestMemBlock(blockAddress, _oneBlockBuff); _oneBlockBuffAddress = blockAddress; // fetch the whole block
        if(!_errGood(err)) { NT3H2x11debugPrint("_setBytesInBlock() read/write error!"); _oneBlockBuffAddress = NT3H2x11_INVALID_MEMA; return(err); }
      }
      if((blockAddress == NT3H2x11_I2C_ADDR_CHANGE_MEMA) && (bytesInBlockStart != NT3H2x11_I2C_ADDR_CHANGE_MEMA_BYTE)) { _oneBlockBuff[NT3H2x11_I2C_ADDR_CHANGE_MEMA_BYTE] = (slaveAddress<<1); } // I2C address byte reads as manufacturer ID
    ...
    if(!_errGood(err)) { NT3H2x11debugPrint("_setBytesInBlock() write error!"); }
    return(err); // err should always be OK, if it makes it to this point
  }
  /**
   ...
   */
  template<typename T> 
  NT3H2x11_ERR_RETURN_TYPE _setValInBlock(uint8_t blockAddress, uint8_t bytesInBlockStart, T newVal, bool writeMSBfirst=true, bool useCache=false) {
    NT3H2x11_ERR_RETURN_TYPE err = NT3H2x11_ERR_RETURN_TYPE_OK;
    if( ! (useCache && (blockAddress == _oneBlockBuffAddress))) { // normally true
      err = requestMemBlock(blockAddress, _oneBlockBuff); _oneBlockBuffAddress = blockAddress; // fetch the whole block
      if(!_errGood(err)) { NT3H2x11debugPrint("_setValInBlock() read/write error!"); _oneBlockBuffAddress = NT3H2x11_INVALID_MEMA; return(err); }
    }
    if((blockAddress == NT3H2x11_I2C_ADDR_CHANGE_MEMA) && (bytesInBlockStart != NT3H2x11_I2C_ADDR_CHANGE_MEMA_BYTE)) { _oneBlockBuff[NT3H2x11_I2C_ADDR_CHANGE_MEMA_BYTE] = (slaveAddress<<1); } // I2C address byte reads as manufacturer ID
    if(blockAddress == NT3H2x11_PWD_MEMA) { // (same as NT3H2x11_PACK_MEMA) the password and PACK read as all 0's (for obvious reasons)
...
/////////////////////////////////////////////////////////////////////////////////////// set functions: //////////////////////////////////////////////////////////
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setI2Caddress(uint8_t newAddress, bool useCache=false) { // does not seem to work (yet)
    NT3H2x11_ERR_RETURN_TYPE err = _setValInBlock<uint8_t>(NT3H2x11_I2C_ADDR_CHANGE_MEMA, NT3H2x11_I2C_ADDR_CHANGE_MEMA_BYTE, newAddress<<1, true, useCache);
    if(_errGood(err)) { slaveAddress = newAddress; } // update this object's address byte ONLY IF the transfer seemed to go as intended
    else { NT3H2x11debugPrint("setI2Caddress() failed!"); }
    return(err);
  }

  /**
   * overwrite the Capability Container (also mentioned as NDEF thingy) with bytes
   * @param writeBuff 4 byte buffer to write to the CC bytes (i stronly recommend NT3H2x11_CAPA_CONT_DEFAULT[is2kVariant])
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setCC(uint8_t writeBuff[], bool useCache = false) { return(_setBytesInBlock(NT3H2x11_CAPA_CONT_MEMA, NT3H2x11_CAPA_CONT_MEMA_BYTES_START, 4, writeBuff, useCache)); }
  /**
   * overwrite the Capability Container (also mentioned as NDEF thingy) with a 4byte value
   * @param newVal 4 byte value (little-endian) to write to the CC bytes (i stronly recommend NT3H2x11_CAPA_CONT_DEFAULT_uint32_t[is2kVariant])
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setCC(uint32_t newVal, bool useCache = false) { return(_setValInBlock<uint32_t>(NT3H2x11_CAPA_CONT_MEMA, NT3H2x11_CAPA_CONT_MEMA_BYTES_START, newVal, true, useCache)); }

  ///////////////////////////////////// Session register set functions: /////////////////////////////////////
  /**
   * overwrite the (whole) NC_REG Session register
   * @param newVal (see NT3H2x11_NC_REG_xxx_bits defines at top for contents)
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_NC_REG(uint8_t newVal) { return(writeSessRegByte(NT3H2x11_COMN_REGS_NC_REG_BYTE, newVal)); }
  /**
   * overwrite FD_OFF bits from the NC_REG Session register
   * @param newVal FD_OFF determines the behaviour of the FD pin (falling)
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_NC_FD_OFF(NT3H2x11_FD_OFF_ENUM newVal) { return(writeSessRegByte(NT3H2x11_COMN_REGS_NC_REG_BYTE, static_cast<uint8_t>(newVal) << 4, NT3H2x11_NC_REG_FD_OFF_bits)); }
  /**
   * overwrite FD_ON bits from the NC_REG Session register
   * @param newVal FD_ON determines the behaviour of the FD pin (rising)
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_NC_FD_ON(NT3H2x11_FD_ON_ENUM newVal) { return(writeSessRegByte(NT3H2x11_COMN_REGS_NC_REG_BYTE, static_cast<uint8_t>(newVal) << 2, NT3H2x11_NC_REG_FD_ON_bits)); }
  /**
   * (private) overwrite one arbetrary bit from the NC_REG Session register
   * @param mask which bit to target (see NT3H2x11_NC_REG_xxx_bits defines at top for contents)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_NC_PTHRU(bool newVal) { return(_setSess_NC_oneBit(NT3H2x11_NC_REG_PTHRU_bits, newVal)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_NC_MIRROR(bool newVal) { return(_setSess_NC_oneBit(NT3H2x11_NC_REG_MIRROR_bits, newVal)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_NC_DIR(bool newVal) { return(_setSess_NC_oneBit(NT3H2x11_NC_REG_DIR_bits, newVal)); } // (just a macro)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_LAST_NDEF_BLOCK(uint8_t newVal) { return(writeSessRegByte(NT3H2x11_COMN_REGS_LAST_NDEF_BLOCK_BYTE, newVal)); }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_SRAM_MIRROR_BLOCK(uint8_t newVal) { return(writeSessRegByte(NT3H2x11_COMN_REGS_SRAM_MIRROR_BLOCK_BYTE, newVal)); }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_WDTraw(uint8_t writeBuff[]) {
    NT3H2x11_ERR_RETURN_TYPE err = writeSessRegByte(NT3H2x11_COMN_REGS_WDT_MS_BYTE, writeBuff[0]);
    if(!_errGood(err)) { return(err); } // if the first one failed, return that error
    return(writeSessRegByte(NT3H2x11_COMN_REGS_WDT_MS_BYTE, writeBuff[1]));
  }
  /**
   * overwrite the WatchDog Timer threshold (raw) in the Session registers
   * @param newVal the WatchDog Timer threshold as a uint16_t, (multiply with NT3H2x11_WDT_RAW_TO_MICROSECONDS to get microseconds)
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_WDTraw(uint16_t newVal) {
    uint8_t* bytePtrToNewVal = (uint8_t*) &newVal;
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setSess_WDT(float newVal) { return(setSess_WDTraw(constrain(newVal,0,(((float)0xFFFF)*NT3H2x11_WDT_RAW_TO_MICROSECONDS)) / NT3H2x11_WDT_RAW_TO_MICROSECONDS)); } // (just a macro)
  ...
  //  * @param newVal (see NT3H2x11_NS_REG_xxx_bits defines at top for contents)
  //  * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
  //  */
  // NT3H2x11_ERR_RETURN_TYPE setNS_REG(uint8_t newVal) { return(writeSessRegByte(NT3H2x11_SESS_REGS_NS_REG_BYTE, newVal, 0b01000100)); }
  /**
   ...
   * @param mask which bit to target (see NT3H2x11_NS_REG_xxx_bits defines at top for contents)
   * @param newBitVal LSBit boolean, to be shifted by the mask to the correct position
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE _setNS_oneBit(uint8_t mask, bool newBitVal) { return(writeSessRegByte(NT3H2x11_SESS_REGS_NS_REG_BYTE, newBitVal ? mask : 0, mask)); }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setNS_I2C_LOCKED(bool newVal) { return(_setNS_oneBit(NT3H2x11_NS_REG_I2C_LOCKED_bits, newVal)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE clear_EEPROM_WR_ERR() { return(_setNS_oneBit(NT3H2x11_NS_REG_EPR_WR_ERR_bits, 0)); } // (just a macro)

  ///////////////////////////////////// Configuration register set functions: /////////////////////////////////////
  /**
   * (private) overwrite an arbetrary value in the Configuration registers
   * @tparam T type of data to write
   * @param bytesInBlockStart which register (use NT3H2x11_CONF_SESS_REGS_ENUM enum)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE _setConfRegBits(NT3H2x11_CONF_SESS_REGS_ENUM byteInBlock, uint8_t newVal, uint8_t mask, bool useCache=false) {
    return(_setBitsInBlock(NT3H2x11_CONF_REGS_MEMA, byteInBlock, newVal, mask, useCache)); // (just a macro) (NOTE: 1x01 to 2x11 change)
  }
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_NC_REG(uint8_t newVal, bool useCache=false) { return(_setConfRegVal<uint8_t>(NT3H2x11_COMN_REGS_NC_REG_BYTE, newVal, false, useCache)); } // (NOTE: 1x01 to 2x11 change)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_NC_FD_OFF(NT3H2x11_FD_OFF_ENUM newVal, bool useCache=false) {
    return(_setConfRegBits(NT3H2x11_COMN_REGS_NC_REG_BYTE, static_cast<uint8_t>(newVal) << 4, NT3H2x11_NC_REG_FD_OFF_bits, useCache)); }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_NC_FD_ON(NT3H2x11_FD_ON_ENUM newVal, bool useCache=false) {
    return(_setConfRegBits(NT3H2x11_COMN_REGS_NC_REG_BYTE, static_cast<uint8_t>(newVal) << 2, NT3H2x11_NC_REG_FD_ON_bits, useCache)); }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE _setConf_NC_oneBit(uint8_t mask, bool newBitVal, bool useCache=false) {
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_NC_NFCS_I2C_RST(bool newVal, bool useCache=false) { return(_setConf_NC_oneBit(NT3H2x11_NC_REG_SIL_SRST_bits, newVal, useCache)); } // (just a macro)
  /**
   * overwrite PTHRU_ON_OFF bit from the NC_REG Configuration register
   * @param newVal PTHRU_ON_OFF bit (bool)     PTHRU_ON_OFF enables Pass-Through mode
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_NC_PTHRU(bool newVal, bool useCache=false) { return(_setConf_NC_oneBit(NT3H2x11_NC_REG_PTHRU_bits, newVal, useCache)); } // (just a macro)
  /**
   * overwrite SRAM_MIRROR_ON_OFF bit from the NC_REG Configuration register
   * @param newVal SRAM_MIRROR_ON_OFF bit (bool)     SRAM_MIRROR_ON_OFF enables Memory-Mirror mode
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_NC_MIRROR(bool newVal, bool useCache=false) { return(_setConf_NC_oneBit(NT3H2x11_NC_REG_MIRROR_bits, newVal, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_NC_DIR(bool newVal, bool useCache=false) { return(_setConf_NC_oneBit(NT3H2x11_NC_REG_DIR_bits, newVal, useCache)); } // (just a macro)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_LAST_NDEF_BLOCK(uint8_t newVal, bool useCache=false) { return(_setConfRegVal<uint8_t>(NT3H2x11_COMN_REGS_LAST_NDEF_BLOCK_BYTE, newVal, false, useCache)); }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_SRAM_MIRROR_BLOCK(uint8_t newVal, bool useCache=false) { return(_setConfRegVal<uint8_t>(NT3H2x11_COMN_REGS_SRAM_MIRROR_BLOCK_BYTE, newVal, false, useCache)); }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_WDTraw(uint8_t writeBuff[], bool useCache=false) {
    return(_setBytesInBlock(NT3H2x11_CONF_REGS_MEMA, NT3H2x11_COMN_REGS_WDT_LS_BYTE, 2, writeBuff, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_WDTraw(uint16_t newVal, bool useCache=false) { return(_setConfRegVal<uint16_t>(NT3H2x11_COMN_REGS_WDT_LS_BYTE, newVal, false, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setConf_WDT(float newVal, bool useCache=false) { return(setConf_WDTraw(constrain(newVal,0,(((float)0xFFFF)*NT3H2x11_WDT_RAW_TO_MICROSECONDS)) / NT3H2x11_WDT_RAW_TO_MICROSECONDS, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE set_I2C_CLOCK_STR(bool newVal, bool useCache=false) { return(_setConfRegBits(NT3H2x11_COMN_REGS_I2C_CLOCK_STR_BYTE, newVal, 0x01, useCache)); } // (just a macro)

  #ifdef NT3H2x11_unlock_burning // functions that can have a permanent consequence)
     ...
     * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
     */
    NT3H2x11_ERR_RETURN_TYPE burnRegLockI2C(bool useCache=false) { return(_setConfRegVal<uint8_t>(NT3H2x11_CONF_REGS_REG_LOCK_BYTE, NT3H2x11_NC_REG_LOCK_I2C_bits, false, useCache)); }
    /**
     * disables writing to the Configuration register bytes from RF PERMANENTLY
     * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
     * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
     */
    NT3H2x11_ERR_RETURN_TYPE burnRegLockRF(bool useCache=false) { return(_setConfRegVal<uint8_t>(NT3H2x11_CONF_REGS_REG_LOCK_BYTE, NT3H2x11_NC_REG_LOCK_RF_bits, false, useCache)); }
  #endif // NT3H2x11_unlock_burning

  ///////////////////////////////////// password (related) set functions: /////////////////////////////////////
  ...
  /**
   * overwrite the AUTH0 byte. NOTE: make absolutely sure you set a password (PWD) (and also set a PACK?) before(!) making this value <= 0xEB (which activates password protection)
   * @param newVal address of first block to be protected by password. > 0xEB disables password protection, minimum value is TBD, but assume 0x02 for now (as per page 16 of datasheet)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setAUTH0(uint8_t newVal, bool useCache=false) { return(_setValInBlock<uint8_t>(NT3H2x11_AUTH0_MEMA, NT3H2x11_AUTH0_MEMA_BYTE, newVal, true, useCache)); }
  /**
   * overwrite the (whole) ACCESS byte (for password stuff)
   * @param newVal (see NT3H2x11_ACCESS_xxx_bits defines at top for contents)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setACCESS(uint8_t newVal, bool useCache=false) { return(_setValInBlock<uint8_t>(NT3H2x11_ACCESS_MEMA, NT3H2x11_ACCESS_MEMA_BYTE, newVal, true, useCache)); }
  /**
   * overwrite NFC_PROT bit in the ACCESS byte (for password stuff)
   * @param newVal NFC_PROT determines whether read-access is ALSO password protected (instead of only write-access) (from NFC)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setACCESS_NFC_PROT(bool newVal, bool useCache=false) {
  ...
  /**
   * overwrite NFC_DIS_SEC1 bit in the ACCESS byte (for password stuff)
   * @param newVal NFC_DIS_SEC1 disables sector 1 (NFC), effectively making the 2k IC the same as the 1k
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setACCESS_NFC_DIS_SEC1(bool newVal, bool useCache=false) {
  ...
  /**
   * overwrite AUTHLIM bits in the ACCESS byte (for password stuff)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setACCESS_AUTHLIMraw(uint8_t newVal, bool useCache=false) {
  ...
  /**
   * disable AUTHLIM (the limit on password auth attempts, which (silently) locks(/destroys????) data when password was wrong too many times)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setACCESS_AUTHLIM_disabled(bool useCache=false) { return(setACCESS_AUTHLIMraw(0, useCache)); } // (just a macro)
  /**
   * overwrite the PWD (password) (if allowed)
   * @param writeBuff 4 byte buffer containing new password. Password should be LSByte first, see page 36 of datasheet
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPWD(uint8_t writeBuff[], bool useCache=false) {
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPACK(uint16_t newVal, bool useCache=false) {
    return(_setValInBlock<uint16_t>(NT3H2x11_PACK_MEMA, NT3H2x11_PACK_MEMA_BYTES_START, newVal, false, useCache)); } // (just a macro)
  
  /**
   * (private) write the contents of _passwordAndPackBuff to the device. Setting the password (if AUTH0 is >0xEB of course)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE _writePWD_and_PACK(bool useCache=false) {
    return(_setBytesInBlock(NT3H2x11_PWD_MEMA, NT3H2x11_PWD_MEMA_BYTES_START, 6, _passwordAndPackBuff, useCache));
  }
  /**
   * overwrite the password and PACK (password acknowledge) at the same time (to avoid the complications of chaching one value while writing partial blocks)
   * @param passwordBuff 4 byte buffer containing new password. Password should be LSByte first, see page 36 of datasheet
   * @param PACKbuff 2 byte buffer containing password acknowledge value. I'm currently not 100% sure what this does...
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPWD_and_PACK(uint8_t passwordBuff[], uint8_t PACKbuff[], bool useCache=false) {
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPWD_and_PACK(uint32_t password, uint16_t PACK, bool useCache=false) {
  ...
  /**
   * overwrite whole block 0x39 (which holds the password stuff) at once. This is the best way to ensure all settings are written correctly
   * @param blockToWrite is a special struct which will ensure the whole block gets written all at once
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE writePasswordBlock(passwordBlock& blockToWrite) {
  ...
  /**
   * overwrite the (whole) PT_I2C byte (for password stuff)
   * @param newVal (see NT3H2x11_PT_I2C_xxx_bits defines at top for contents)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPT_I2C(uint8_t newVal, bool useCache=false) { return(_setValInBlock<uint8_t>(NT3H2x11_PT_I2C_MEMA, NT3H2x11_PT_I2C_MEMA_BYTE, newVal, true, useCache)); }
  /**
   * overwrite 2K_PROT bit in the PT_I2C byte (for password stuff)
   * @param newVal 2K_PROT enables password protection for sector 1 (only on 2k version)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPT_I2C_2K_PROT(bool newVal, bool useCache=false) {
  ...
  /**
   * overwrite SRAM_PROT bit in the PT_I2C byte (for password stuff)
   * @param newVal SRAM_PROT enables password protection for SRAM (for pass-through and mirror mode) (does not apply to I2C?)
   * @param useCache (optional!, not recommended, use at own discretion) use data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPT_I2C_SRAM_PROT(bool newVal, bool useCache=false) {
  ...
  /**
   * overwrite I2C_PROT bits in the PT_I2C byte (for password stuff)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE setPT_I2C_I2C_PROT(uint8_t newVal, bool useCache=false) {
...
/////////////////////////////////////////////////////////////////////////////////////// get functions: //////////////////////////////////////////////////////////
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getUID(uint8_t readBuff[], bool useCache=false) { return(_getBytesFromBlock(NT3H2x11_SERIAL_NR_MEMA, NT3H2x11_SERIAL_NR_MEMA_BYTES_START, 7, readBuff, useCache)); } // (just a macro)
  /**
   ...
   * @param readBuff 4 byte buffer to put the results in (results should match NT3H2x11_CAPA_CONT_DEFAULT)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getCC(uint8_t readBuff[], bool useCache=false) { return(_getBytesFromBlock(NT3H2x11_CAPA_CONT_MEMA, NT3H2x11_CAPA_CONT_MEMA_BYTES_START, 4, readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the Capability Container (also mentioned as NDEF thingy) (this version DOES NOT let you check for I2C errors)
   * @param readMSBfirst whether to read the MSByte first or the LSByte first (Big/Little-endian respectively)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the Capability Container as a uint32_t. result should match NT3H2x11_CAPA_CONT_DEFAULT_uint32_t
   */
  uint32_t getCC(bool readMSBfirst=true, bool useCache=false) { return(_getValFromBlock<uint32_t>(NT3H2x11_CAPA_CONT_MEMA, NT3H2x11_CAPA_CONT_MEMA_BYTES_START, readMSBfirst, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getATQA(uint8_t readBuff[], bool useCache=false) { return(_getBytesFromBlock(NT3H2x11_ATQA_MEMA, NT3H2x11_ATQA_MEMA_BYTES_START, 2, readBuff, useCache)); } // (just a macro)
  /**
   ...
   */
  uint16_t getATQA(bool readMSBfirst=true, bool useCache=false) { return(_getValFromBlock<uint16_t>(NT3H2x11_ATQA_MEMA, NT3H2x11_ATQA_MEMA_BYTES_START, readMSBfirst, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getSAK(uint8_t readBuff[], bool useCache=false) { return(_getBytesFromBlock(NT3H2x11_SAK_MEMA, NT3H2x11_SAK_MEMA_BYTE, 1, readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the SAK byte (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the SAK byte
   */
  uint8_t getSAK(bool useCache=false) { return(_getValFromBlock<uint8_t>(NT3H2x11_SAK_MEMA, NT3H2x11_SAK_MEMA_BYTE, true, useCache)); } // (just a macro)
   ...
   * @param readBuff byte reference to put the result in (see NT3H2x11_NC_REG_xxx_bits defines at top for contents)
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getSess_NC_REG(uint8_t& readBuff) { return(requestSessRegByte(NT3H2x11_COMN_REGS_NC_REG_BYTE, readBuff)); } // (just a macro)
  /**
   * retrieve the (whole) NC_REG Session register (this version DOES NOT let you check for I2C errors)
   * @return the (whole) NC_REG (see NT3H2x11_NC_REG_xxx_bits defines at top for contents)
   */
  uint8_t getSess_NC_REG() {
    uint8_t readBuff;   NT3H2x11_ERR_RETURN_TYPE err = getSess_NC_REG(readBuff);
    if(!_errGood(err)) { NT3H2x11debugPrint("getSess_NC_REG() read/write error!"); }
    return(readBuff);
  }
  /**
   ...
   */
  bool getSess_NC_PTHRU() { return((getSess_NC_REG() & NT3H2x11_NC_REG_PTHRU_bits) != 0); } // (just a macro)
  /**
   ...
   */
  NT3H2x11_FD_OFF_ENUM getSess_NC_FD_OFF() { return(static_cast<NT3H2x11_FD_OFF_ENUM>((getSess_NC_REG() & NT3H2x11_NC_REG_FD_OFF_bits) >> 4)); } // (just a macro)
  /**
   ...
   */
  NT3H2x11_FD_ON_ENUM getSess_NC_FD_ON() { return(static_cast<NT3H2x11_FD_ON_ENUM>((getSess_NC_REG() & NT3H2x11_NC_REG_FD_ON_bits) >> 2)); } // (just a macro)
  /**
   ...
   */
  bool getSess_NC_MIRROR() { return((getSess_NC_REG() & NT3H2x11_NC_REG_MIRROR_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getSess_NC_DIR() { return((getSess_NC_REG() & NT3H2x11_NC_REG_DIR_bits) != 0); } // (just a macro)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getSess_LAST_NDEF_BLOCK(uint8_t& readBuff) { return(requestSessRegByte(NT3H2x11_COMN_REGS_LAST_NDEF_BLOCK_BYTE, readBuff)); } // (just a macro)
  /**
  ...
  uint8_t getSess_LAST_NDEF_BLOCK() {
    uint8_t readBuff;   NT3H2x11_ERR_RETURN_TYPE err = getSess_LAST_NDEF_BLOCK(readBuff);
    if(!_errGood(err)) { NT3H2x11debugPrint("getSess_LAST_NDEF_BLOCK() read/write error!"); }
    return(readBuff);
  }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getSess_SRAM_MIRROR_BLOCK(uint8_t& readBuff) { return(requestSessRegByte(NT3H2x11_COMN_REGS_SRAM_MIRROR_BLOCK_BYTE, readBuff)); } // (just a macro)
  /**
  ...
  uint8_t getSess_SRAM_MIRROR_BLOCK() {
    uint8_t readBuff;   NT3H2x11_ERR_RETURN_TYPE err = getSess_SRAM_MIRROR_BLOCK(readBuff);
    if(!_errGood(err)) { NT3H2x11debugPrint("getSess_SRAM_MIRROR_BLOCK() read/write error!"); }
    return(readBuff);
  }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getSess_WDTraw(uint8_t readBuff[]) {
    NT3H2x11_ERR_RETURN_TYPE err = requestSessRegByte(NT3H2x11_COMN_REGS_WDT_LS_BYTE, readBuff[0]);
    if(!_errGood(err)) { return(err); } // if the first one failed, return that error
    return(requestSessRegByte(NT3H2x11_COMN_REGS_WDT_MS_BYTE, readBuff[1]));
  }
  /**
   * retrieve the WatchDog Timer threshold (raw) from the Session registers (this version DOES NOT let you check for I2C errors)
   * @return the WatchDog Timer threshold as a uint16_t, multiply with NT3H2x11_WDT_RAW_TO_MICROSECONDS to get microseconds
   */
  uint16_t getSess_WDTraw() {
    ...
    NT3H2x11_ERR_RETURN_TYPE err = getSess_WDTraw(bytePtrToReturnVal);
    if(!_errGood(err)) { NT3H2x11debugPrint("getSess_WDTraw() read/write error!"); }
    return(returnVal);
  }
  /**
   ...
   */
  float getSess_WDT() { return((float) getSess_WDTraw() * NT3H2x11_WDT_RAW_TO_MICROSECONDS); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getSess_I2C_CLOCK_STR(uint8_t& readBuff) { return(requestSessRegByte(NT3H2x11_COMN_REGS_I2C_CLOCK_STR_BYTE, readBuff)); } // (just a macro)
  /**
  ...
  uint8_t getSess_I2C_CLOCK_STR() {
    uint8_t readBuff;   NT3H2x11_ERR_RETURN_TYPE err = getSess_I2C_CLOCK_STR(readBuff);
    if(!_errGood(err)) { NT3H2x11debugPrint("getSess_I2C_CLOCK_STR() read/write error!"); }
   ...
   */
  bool getSess_NEG_AUTH_REACHED() { return((getSess_I2C_CLOCK_STR() & NT3H2x11_NEG_AUTH_REACHED_bits) != 0); } // (just a macro)


  /**
   * retrieve the (whole) NS_REG Session register (this version of the function lets you check for I2C errors)
   * @param readBuff byte reference to put the result in (see NT3H2x11_NS_REG_xxx_bits defines at top for contents)
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getNS_REG(uint8_t& readBuff) { return(requestSessRegByte(NT3H2x11_SESS_REGS_NS_REG_BYTE, readBuff)); } // (just a macro)
  /**
   * retrieve the (whole) NS_REG Session register (this version DOES NOT let you check for I2C errors)
   * @return the (whole) NS_REG (see NT3H2x11_NS_REG_xxx_bits defines at top for contents)
   */
  uint8_t getNS_REG() {
    uint8_t readBuff;   NT3H2x11_ERR_RETURN_TYPE err = getNS_REG(readBuff);
    if(!_errGood(err)) { NT3H2x11debugPrint("getNS_REG() read/write error!"); }
    return(readBuff);
  }
  /**
   ...
   */
  bool getNS_NDEF_DATA_READ() { return((getNS_REG() & NT3H2x11_NS_REG_NDEF_READ_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getNS_I2C_LOCKED() { return((getNS_REG() & NT3H2x11_NS_REG_I2C_LOCKED_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getNS_RF_LOCKED() { return((getNS_REG() & NT3H2x11_NS_REG_RF_LOCKED_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getNS_SRAM_I2C_READY() { return((getNS_REG() & NT3H2x11_NS_REG_PTHRU_IN_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getNS_SRAM_RF_READY() { return((getNS_REG() & NT3H2x11_NS_REG_PTHRU_OUT_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getNS_EEPROM_WR_ERR() { return((getNS_REG() & NT3H2x11_NS_REG_EPR_WR_ERR_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getNS_EEPROM_WR_BUSY() { return((getNS_REG() & NT3H2x11_NS_REG_EPR_WR_BSY_bits) != 0); } // (just a macro)
  /**
   ...
   */
  bool getNS_RF_FIELD_PRESENT() { return((getNS_REG() & NT3H2x11_NS_REG_RF_FIELD_bits) != 0); } // (just a macro)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE _getConfRegBytes(NT3H2x11_CONF_SESS_REGS_ENUM bytesInBlockStart, uint8_t bytesToRead, uint8_t readBuff[], bool useCache = false)
    { return(_getBytesFromBlock(NT3H2x11_CONF_REGS_MEMA, bytesInBlockStart, bytesToRead, readBuff, useCache)); }
  /**
   ...
   */
  template<typename T> 
  T _getConfRegVal(NT3H2x11_CONF_SESS_REGS_ENUM bytesInBlockStart, bool readMSBfirst=false, bool useCache=false)
    { return(_getValFromBlock<T>(NT3H2x11_CONF_REGS_MEMA, bytesInBlockStart, readMSBfirst, useCache)); }
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getConf_NC_REG(uint8_t& readBuff, bool useCache=false) { return(_getConfRegBytes(NT3H2x11_COMN_REGS_NC_REG_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the (whole) NC_REG Configuration register (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the (whole) NC_REG (see NT3H2x11_NC_REG_xxx_bits defines at top for contents)
   */
  uint8_t getConf_NC_REG(bool useCache=false) { return(_getConfRegVal<uint8_t>(NT3H2x11_COMN_REGS_NC_REG_BYTE, true, useCache)); } // (just a macro)
  /**
  ...
  /**
   * retrieve PTHRU_ON_OFF bit from the NC_REG Configuration register
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the PTHRU_ON_OFF bit (bool)     PTHRU_ON_OFF enables Pass-Through mode
   */
  bool getConf_NC_PTHRU(bool useCache=false) { return((getConf_NC_REG(useCache) & NT3H2x11_NC_REG_PTHRU_bits) != 0); } // (just a macro)
  /**
   * retrieve FD_OFF bits from the NC_REG Configuration register
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the FD_OFF bits (2)     FD_OFF determines the behaviour of the FD pin (falling)
   */
  NT3H2x11_FD_OFF_ENUM getConf_NC_FD_OFF(bool useCache=false) { return(static_cast<NT3H2x11_FD_OFF_ENUM>((getConf_NC_REG(useCache) & NT3H2x11_NC_REG_FD_OFF_bits) >> 4)); } // (just a macro)
  /**
   * retrieve FD_ON bits from the NC_REG Configuration register
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the FD_ON bits (2)     FD_ON determines the behaviour of the FD pin (rising)
   */
  NT3H2x11_FD_ON_ENUM getConf_NC_FD_ON(bool useCache=false) { return(static_cast<NT3H2x11_FD_ON_ENUM>((getConf_NC_REG(useCache) & NT3H2x11_NC_REG_FD_ON_bits) >> 2)); } // (just a macro)
  /**
   * retrieve SRAM_MIRROR_ON_OFF bit from the NC_REG Configuration register
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the SRAM_MIRROR_ON_OFF bit (bool)     SRAM_MIRROR_ON_OFF enables Memory-Mirror mode
   */
  bool getConf_NC_MIRROR(bool useCache=false) { return((getConf_NC_REG(useCache) & NT3H2x11_NC_REG_MIRROR_bits) != 0); } // (just a macro)
  /**
   * retrieve TRANSFER_DIR/PTHRU_DIR bit from the NC_REG Configuration register
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the TRANSFER_DIR/PTHRU_DIR bit (bool)     TRANSFER_DIR/PTHRU_DIR determines the direction of data in Pass-Through mode, or can disable RF write-access otherwise
   */
  bool getConf_NC_DIR(bool useCache=false) { return((getConf_NC_REG(useCache) & NT3H2x11_NC_REG_DIR_bits) != 0); } // (just a macro)
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getConf_LAST_NDEF_BLOCK(uint8_t& readBuff, bool useCache=false) { return(_getConfRegBytes(NT3H2x11_COMN_REGS_LAST_NDEF_BLOCK_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the LAST_NDEF_BLOCK byte from the Configuration registers (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the LAST_NDEF_BLOCK byte
   */
  uint8_t getConf_LAST_NDEF_BLOCK(bool useCache=false) { return(_getConfRegVal<uint8_t>(NT3H2x11_COMN_REGS_NC_REG_BYTE, true, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getConf_SRAM_MIRROR_BLOCK(uint8_t& readBuff, bool useCache=false) { return(_getConfRegBytes(NT3H2x11_COMN_REGS_SRAM_MIRROR_BLOCK_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the SRAM_MIRROR_BLOCK byte from the Configuration registers (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the SRAM_MIRROR_BLOCK byte
   */
  uint8_t getConf_SRAM_MIRROR_BLOCK(bool useCache=false) { return(_getConfRegVal<uint8_t>(NT3H2x11_COMN_REGS_SRAM_MIRROR_BLOCK_BYTE, true, useCache)); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getConf_WDTraw(uint8_t readBuff[], bool useCache=false) { return(_getConfRegBytes(NT3H2x11_COMN_REGS_WDT_LS_BYTE, 2, readBuff, useCache)); } // (just a macro)
  /**
   ...
   * @return the WatchDog Timer threshold as a uint16_t, multiply with NT3H2x11_WDT_RAW_TO_MICROSECONDS to get microseconds
   */
  uint16_t getConf_WDTraw(bool useCache=false) { return(_getConfRegVal<uint16_t>(NT3H2x11_COMN_REGS_WDT_LS_BYTE, false, useCache)); }
  /**
   ...
   * @return the WatchDog Timer threshold in microseconds
   */
  float getConf_WDT(bool useCache=false) { return((float) getConf_WDTraw(useCache) * NT3H2x11_WDT_RAW_TO_MICROSECONDS); } // (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getConf_I2C_CLOCK_STR(uint8_t& readBuff, bool useCache=false) { return(_getConfRegBytes(NT3H2x11_COMN_REGS_I2C_CLOCK_STR_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   ...
   */
  bool getConf_I2C_CLOCK_STR(bool useCache=false) { return(_getConfRegVal<uint8_t>(NT3H2x11_COMN_REGS_I2C_CLOCK_STR_BYTE, true, useCache)); } // (templated as uint8_t then convert to bool) (just a macro)
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getREG_LOCK(uint8_t& readBuff, bool useCache=false) { return(_getConfRegBytes(NT3H2x11_CONF_REGS_REG_LOCK_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the I2C clock stretching bit from the Configuration registers (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the REG_LOCK byte (see NT3H2x11_NC_REG_LOCK_xxx_bits defines up top for contents)
   */
  uint8_t getREG_LOCK(bool useCache=false) { return(_getConfRegVal<uint8_t>(NT3H2x11_CONF_REGS_REG_LOCK_BYTE, true, useCache)); } // (just a macro)


  ///////////////////////////////////// password (related) get functions: /////////////////////////////////////
  /**
   * retrieve the AUTH0 byte (the address of first block to be protected by password) (this version of the function lets you check for I2C errors)
   * @param readBuff byte reference to put the result in
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getAUTH0(uint8_t& readBuff, bool useCache=false) { return(_getBytesFromBlock(NT3H2x11_AUTH0_MEMA, NT3H2x11_AUTH0_MEMA_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the AUTH0 byte (the address of first block to be protected by password) (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the address of first block to be protected by password. > 0xEB disables password protection, minimum value is TBD, but assume 0x02 for now (as per page 16 of datasheet)
   */
  uint8_t getAUTH0(bool useCache=false) { return(_getValFromBlock<uint8_t>(NT3H2x11_AUTH0_MEMA, NT3H2x11_AUTH0_MEMA_BYTE, true, useCache)); } // (just a macro)
  /**
   * retrieve the (whole) ACCESS byte (for password stuff) (this version of the function lets you check for I2C errors)
   * @param readBuff byte reference to put the result in (see NT3H2x11_ACCESS_xxx_bits defines at top for contents)
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getACCESS(uint8_t& readBuff, bool useCache=false) { return(_getBytesFromBlock(NT3H2x11_ACCESS_MEMA, NT3H2x11_ACCESS_MEMA_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the (whole) ACCESS byte (for password stuff) (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the (whole) ACCESS byte (for password stuff) (see NT3H2x11_ACCESS_xxx_bits defines at top for contents)
   */
  uint8_t getACCESS(bool useCache=false) { return(_getValFromBlock<uint8_t>(NT3H2x11_ACCESS_MEMA, NT3H2x11_ACCESS_MEMA_BYTE, true, useCache)); } // (just a macro)
  /**
   * retrieve NFC_PROT bit from the ACCESS byte (for password stuff)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   ...
   */
  bool getACCESS_NFC_PROT(bool useCache=false) { return((getACCESS(useCache) & NT3H2x11_ACCESS_NFC_PROT_bits) != 0); } // (just a macro)
  /**
   * retrieve NFC_DIS_SEC1 bit from the ACCESS byte (for password stuff)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the NFC_DIS_SEC1 bit (bool)     NFC_DIS_SEC1 disables sector 1 (NFC), effectively making the 2k IC the same as the 1k
   */
  bool getACCESS_NFC_DIS_SEC1(bool useCache=false) { return((getACCESS(useCache) & NT3H2x11_ACCESS_NFC_DIS_SEC1_bits) != 0); } // (just a macro)
  /**
   * retrieve AUTHLIM bits from the ACCESS byte (for password stuff)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   ...
   */
  uint8_t getACCESS_AUTHLIMraw(bool useCache=false) { return(getACCESS(useCache) & NT3H2x11_ACCESS_AUTHLIM_bits); } // (just a macro)
  /**
   * retrieve AUTHLIM (value) from the ACCESS byte (for password stuff)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
  ...
  uint8_t getACCESS_AUTHLIM(bool useCache=false) {
    ...
    return(AUTHLIMraw ? (1 << AUTHLIMraw) : 0); // see page 37 of datasheet
  }
  /**
   * retrieve the (whole) PT_I2C byte (for password stuff) (this version of the function lets you check for I2C errors)
   * @param readBuff byte reference to put the result in (see NT3H2x11_PT_I2C_xxx_bits defines at top for contents)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote/read successfully
   */
  NT3H2x11_ERR_RETURN_TYPE getPT_I2C(uint8_t& readBuff, bool useCache=false) { return(_getBytesFromBlock(NT3H2x11_PT_I2C_MEMA, NT3H2x11_PT_I2C_MEMA_BYTE, 1, &readBuff, useCache)); } // (just a macro)
  /**
   * retrieve the (whole) PT_I2C byte (for password stuff) (this version DOES NOT let you check for I2C errors)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the (whole) PT_I2C byte (for password stuff) (see NT3H2x11_PT_I2C_xxx_bits defines at top for contents)
   */
  uint8_t getPT_I2C(bool useCache=false) { return(_getValFromBlock<uint8_t>(NT3H2x11_PT_I2C_MEMA, NT3H2x11_PT_I2C_MEMA_BYTE, true, useCache)); } // (just a macro)
  /**
   * retrieve  bit from the PT_I2C byte (for password stuff)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the  bit (bool)     
   */
  bool getPT_I2C_2K_PROT(bool useCache=false) { return((getPT_I2C(useCache) & NT3H2x11_PT_I2C_2K_PROT_bits) != 0); } // (just a macro)
  /**
   * retrieve  bit from the PT_I2C byte (for password stuff)
   * @param useCache (optional!, not recommended, use at own discretion) fetch data from _oneBlockBuff cache (if possible) instead of actually reading it from I2C (to save a little time).
   * @return the  bit (bool)     
   ...
   * @return the  bits (2)     
   */
  uint8_t getPT_I2C_I2C_PROT(bool useCache=false) { return(getPT_I2C(useCache) & NT3H2x11_PT_I2C_I2C_PROT_bits); } // (just a macro)
...
/////////////////////////////////////////////////////////////////////////////////////// debug functions: //////////////////////////////////////////////////////////
  ...
  bool connectionCheck() {
    uint8_t UID[7];
    NT3H2x11_ERR_RETURN_TYPE err = getUID(UID); // fetch the Serial Number a.k.a. UID, the first byte of which should always be the manufacturer ID
    if(!_errGood(err)) { return(false); } // at this point it will have already printed several debug messages, no need to print another
    return(UID[0] == NT3H2x11_SERIAL_NR_NXP_MF_ID);
  }
  /**
  ...
  bool variantCheck(bool useCache=false) {
    uint8_t readBuff[4];
    NT3H2x11_ERR_RETURN_TYPE err = getCC(readBuff, useCache); // fetch the Serial Number a.k.a. UID, the 3rd byte of which indicates the size of the tag
    if(!_errGood(err)) { NT3H2x11debugPrint("variantCheck() read/write error!"); return(false); }
    bool returnBool = (readBuff[2] == NT3H2x11_CAPA_CONT_DEFAULT[is2kVariant][2]);
    if(!returnBool) { NT3H2x11debugPrint("variantCheck(), CC/NDEF memory size byte did NOT match expectation!"); }
    return(returnBool);
  }
  ...
  //  * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
  //  */
  // NT3H2x11_ERR_RETURN_TYPE printConfig() {
  //   // TODO!
  // }
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE resetConfiguration(bool useCache=false) {
    uint8_t tempArr[6];  for(uint8_t i=0;i<6;i++) { tempArr[i]=NT3H2x11_CONF_REGS_DEFAULT[i]; } // this copy is needed, becuase DEFAULT array is const, and the function only likes non-const data...
    return(_setBytesInBlock(NT3H2x11_CONF_REGS_MEMA, NT3H2x11_COMN_REGS_NC_REG_BYTE, 6, tempArr, useCache)); // set all bytes (except REG_LOCK) to their default value
  }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE saveSessionToConfiguration(bool useCache=false) {
    //// read from Sess:
    uint8_t tempArr[5];   NT3H2x11_ERR_RETURN_TYPE err;
    for(uint8_t i=0;i<5;i++) {
      err = requestSessRegByte(static_cast<NT3H2x11_CONF_SESS_REGS_ENUM>(NT3H2x11_COMN_REGS_NC_REG_BYTE + i), tempArr[i]); // NOTE: if enum values were not contiguous, this would not work
      if(!_errGood(err)) { NT3H2x11debugPrint("saveSessionToConfiguration() read error!"); return(err); }
    }
    //// write to Conf:
    return(_setBytesInBlock(NT3H2x11_CONF_REGS_MEMA, NT3H2x11_COMN_REGS_NC_REG_BYTE, 5, tempArr, useCache));
  }
  /**
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE reloadConfiguration(bool useCache=false) {
    //// read from Sess:
    uint8_t tempArr[6];  NT3H2x11_ERR_RETURN_TYPE err = _getConfRegBytes(NT3H2x11_COMN_REGS_NC_REG_BYTE, 6, tempArr, useCache);
    if(!_errGood(err)) { NT3H2x11debugPrint("reloadConfiguration() read error!"); return(err); }
    //// write to Conf:
    for(uint8_t i=0;i<5;i++) {
      err = writeSessRegByte(static_cast<NT3H2x11_CONF_SESS_REGS_ENUM>(NT3H2x11_COMN_REGS_NC_REG_BYTE + i), tempArr[i]);
      if(!_errGood(err)) { NT3H2x11debugPrint("reloadConfiguration() write error!"); return(err); }
    }
   ...
   * @return (bool or esp_err_t or i2c_status_e, see on defines at top) whether it wrote successfully
   */
  NT3H2x11_ERR_RETURN_TYPE resetCC() { return(setCC(NT3H2x11_CAPA_CONT_DEFAULT_uint32_t[is2kVariant])); } // write it as uint16_t (just a macro)
  // NT3H2x11_ERR_RETURN_TYPE resetCC() { uint8_t tempArr[4]; for(uint8_t i=0;i<4;i++){tempArr[i]=NT3H2x11_CAPA_CONT_DEFAULT[is2kVariant][i];} return(setCC(tempArr)); } // write individual bytes



  //// password stuff: (NOTE: 1x01 to 2x11 change)
  /**
   * store the password and PACK (password acknowledge) in a buffer. This is required to safely write to only-parts-of block 0x39 (which also contains ACCESS and PT_I2C, for example)
   * @param passwordBuff 4 byte buffer containing new password. Password should be LSByte first, see page 36 of datasheet
   * @param PACKbuff 2 byte buffer containing password acknowledge value. I'm currently not 100% sure what this does...
   */
